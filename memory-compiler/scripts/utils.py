...
def wiki_article_exists(link: str) -> bool:
    """Check if a wikilinked article exists on disk (any article directory under knowledge/)."""
    normalized = link.removesuffix(".md")
    if (KNOWLEDGE_DIR / f"{normalized}.md").exists():
        return True
    for article in list_wiki_articles():
        rel = article.relative_to(KNOWLEDGE_DIR).as_posix().removesuffix(".md")
        if rel == normalized:
            return True
    return False
...
def build_index_entry(rel_path: str, summary: str, sources: str, updated: str) -> str:
    ...
    return f"| [[{link}]] | {esc(summary)} | {esc(sources)} | {updated} |"
