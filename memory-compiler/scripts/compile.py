...
from config import AGENTS_FILE, CONCEPTS_DIR, CONNECTIONS_DIR, DAILY_DIR, KNOWLEDGE_DIR, LOG_FILE, now_iso
from utils import (
    file_hash,
    list_raw_files,
    list_wiki_articles,
...
async def compile_daily_log(log_path: Path, state: dict) -> float:
    ...
    # Snapshot outputs before the LLM run so we only mark ingest on real changes.
    log_digest_before = file_hash(LOG_FILE) if LOG_FILE.exists() else None
    article_digests_before = {p.resolve(): file_hash(p) for p in list_wiki_articles()}
...
"""
    ...
    def compilation_produced_output() -> bool:
        if LOG_FILE.exists():
            h = file_hash(LOG_FILE)
            if log_digest_before is None or h != log_digest_before:
                return True
        for p in list_wiki_articles():
            key = p.resolve()
            h = file_hash(p)
            if key not in article_digests_before:
                return True
            if article_digests_before[key] != h:
                return True
        return False

    if not compilation_produced_output():
        msg = (
            "Compilation finished but knowledge/log.md and wiki articles were unchanged; "
            "not updating ingest state (retry or inspect the LLM run)."
        )
        print(f"  Warning: {msg}")
