...
def _qa_markdown_count() -> int:
    """Number of Q&A articles on disk (top-level *.md under QA_DIR)."""
    return len(list(QA_DIR.glob("*.md"))) if QA_DIR.exists() else 0
...
def main():
    ...
    qa_count_before = _qa_markdown_count() if args.file_back else 0
    ...
    if args.file_back:
        ...
        if isinstance(answer, str) and answer.startswith("Error"):
            print(f"Query did not complete successfully: {answer}")
        else:
            qa_count_after = _qa_markdown_count()
            if qa_count_after > qa_count_before:
                print(
                    f"Answer filed under {QA_DIR} "
                    f"({qa_count_after} Q&A articles total; "
                    f"+{qa_count_after - qa_count_before} new).",
                )
            else:
                print(
                    f"Warning: No new Q&A markdown file was detected under {QA_DIR} "
                    f"(count before: {qa_count_before}, after: {qa_count_after}). "
                    "Check the model run, tool permissions, or schema instructions.",
                    file=sys.stderr,
                )
                sys.exit(1)
