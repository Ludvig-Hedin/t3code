...
import { remarkInlineLinks } from "../markdown-inline-links";
...
/**
 * Language identifiers that indicate a shell/terminal code block.
 * When one of these is detected we show a "Run in terminal" button.
 */
...
function MarkdownCodeBlock({ code, language, children, onRunInTerminal }: MarkdownCodeBlockProps) {
  ...
  // Show Run button only for shell-type blocks when a handler is provided
    ...
    <div className="chat-markdown-codeblock">
      ...
      <div className="chat-markdown-codeblock-actions">
        {showRunButton && (
          <button
            type="button"
            className="chat-markdown-run-button"
  ...
  const markdownComponents = useMemo<Components>(
    () => ({
      a({ node: _node, href, ...props }) {
        // Check if this link was auto-generated by remarkInlineLinks for a file path
        const autoLinkKind = (props as Record<string, unknown>)["data-auto-link"] as
          | string
          | undefined;

        if (autoLinkKind === "path") {
          // Auto-detected file path in plain text — open in preferred editor
        ...
        if (autoLinkKind === "url") {
          // Auto-detected URL in plain text — open in browser
          ...
          return (
            <a
              ...
              onClick={(event) => {
                // Use native openExternal when available (Electron / desktop bridge)
                const api = readNativeApi();
                if (api) {
                  event.preventDefault();
                  event.stopPropagation();
                  void api.shell.openExternal(resolvedHref).catch(() => undefined);
                }
                // Otherwise let the default <a> behaviour open the URL in a new tab
              }}
            />
          );
        }

        // Standard markdown link (explicitly authored by the model)
        const targetPath = resolveMarkdownFileLinkTarget(href, cwd);
        if (!targetPath) {
        ...
        if (!className) {
          ...
          if (targetPath) {
            ...
            return (
              <code
                {...props}
                // role="button" + tabIndex makes the element keyboard-reachable (WCAG 2.1 SC 2.1.1).
                role="button"
                tabIndex={0}
                className="cursor-pointer underline decoration-dotted underline-offset-2"
                title={targetPath}
    ...
    [cwd, diffThemeName, isStreaming, onRunInTerminal],
  );
